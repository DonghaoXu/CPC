...
% 	majorticks = majorticks(1:end-1);
    s = sprintf('/%d', majors);
    labels = [num2str((1:majors - 1)'), repmat(s, majors - 1, 1)];
    labels = [' 0 '; labels; ' 1 '];
% 	labels = num2str(majorticks');
	...
	text(lxc, lyc, [repmat('  ', size(labels,1), 1) labels]);
	
	% Plot bottom labels (no b - only a c)
	[lxb, lyb] = terncoords(majorticks, zerocomp, 1-majorticks); % fB = 1-fA
	text(lxb, lyb - yoffset, labels, 'HorizontalAlignment', 'center','VerticalAlignment','top');
...
%     lxa=lxa(1:end-1); lya=lya(1:end-1);
%     lxb=lxb(1:end-1); lyb=lyb(1:end-1);
%     lxc=lxc(1:end-1); lyc=lyc(1:end-1);
	nlabels = size(labels,1)-2;
	for i = 1:nlabels
