function [ref,inters,ia,oi]=refl(inc,stp,yl2,yl1,yr2,yr1,r_g,t_g,r_a,gap,tol,theta)
...
ina=inc;
s=stp;
R=[inc(1),-inc(2);inc(2),inc(1)];
iR=[inc(1),inc(2);-inc(2),inc(1)];
ryl2=(yl2-ones(size(yl2,1),1)*s)*R;
ryl1=(yl1-ones(size(yl1,1),1)*s)*R;
ryr2=(yr2-ones(size(yr2,1),1)*s)*R;
ryr1=(yr1-ones(size(yr1,1),1)*s)*R;
...
if any(any(isfinite(allinter(:,[1,3]))))
    [~,ind]=min(allinter(:,1));
    inters1=allinter(ind,1:2)*iR+s;
    inters2=allinter(ind,3:4)*iR+s;
...
%get the reflected direction
    [ref,ia,oi]=refdir(inc,inters,ind,theta,r_a);
else
    ref=[NaN,NaN];
    inters=[NaN,NaN];
    ia=NaN;
