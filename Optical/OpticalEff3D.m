function [q,varargout]=OpticalEff3D(data,type,surp,x,angle,varargin)
...
[~,id]=ismember(abs(angle),history.IncidentAngles,'rows');
if any(~id)
    f=@(x) fprintf('I don''t have ray tracing data for angle: %.1f',x);
    cellfun(f,mat2cell(angle(~id),sum(~id),1))
    error('Bang!')
end
ids = reshape(1: history.NumberOfRays * size(angle, 1), history.NumberOfRays, []);
id = ids(:, id);
id = id(:);

X=history.x(id, :);
% X(angle(:, 1)<0,:)=-X(angle(:, 1)<0,:);
Y=history.y(id,:);
Z = history.z(id, :);
IA=history.ia(id,:);
ID=history.index(id,:);
